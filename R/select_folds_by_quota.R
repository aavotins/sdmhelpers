...
#' whose combined number of observations yields a selected proportion close
#' to `target_prop`.
#'
#' Candidate subsets are generated using repeated random orderings of the fold
#' groups. For each ordering, groups are greedily added whenever doing so would
#' not cause the cumulative number of selected rows to exceed the integer upper
#' bound. If the resulting subset lies below the lower bound, the function
#' evaluates the unchanged subset together with every subset obtainable by
#' adding one remaining group. These candidates are scored according to their
#' distance from the target row count, with an additional penalty for falling
#' outside the requested bounds, and the best-scoring candidate is retained.
#'
...
#' during at most `max_tries` randomized attempts, according to the scoring rule
#' described below. In some cases, particularly
...
#' within the requested bounds. The procedure is a randomized heuristic rather than
#' evaluating all possible combinations of fold groups. Consequently, a returned
#' solution outside the bounds does not by itself prove that no feasible
#' combination exists.
#'
#' #' Candidate solutions are scored according to their absolute deviation from
#' the target number of selected rows. Solutions falling outside the requested
#' row-count bounds receive an additional penalty equal to their distance
...
#' target_n. Solutions outside the bounds are additionally penalized according
#' to how far they lie outside the permitted interval.
#'
...
#'   solution containing exactly the `target_n` and lying within
...
#'   fold group represented by `"<NA>"` in `selected_fold_ids`. The default is `FALSE`.
#'
...
#'   solution is printed to the console. The returned object is invisible regardless of
#'   this setting. The default is `TRUE`.
#'
#' @return
...
#'
#' @details
#' Selects complete fold groups so that their combined number of rows is close
#' to a target row count derived from the requested sample proportion. Fold groups
#' are never split between the selected and unselected sets.
#'
...
#' optimal combination of folds. By selecting larger value of `max_tries` can increase the
#' probability of finding a lower-scoring (better) combination when many fold groups are
#' available.
#'
