...
You can install the development version of `sdmhelpers` from GitHub
with:
...
- `phenology_overlap_weights()` calculates kernel-density overlap
  between the day-of-year distribution of a target species and multiple
  comparison species, and converts the overlap estimates into normalized
  weights;

- `doy_kde_weights()` for seasonal phenological weights calculated for
  day of inventory or trapping as compared to species (registration)
  activity;

- `torus_overlap_many()` for seasonal and diel overlap between reference
  species and possibly many other species;

- `select_folds_by_quota()` for selecting presences as complete fold
  groups;

- `select_joint_folds()` for coordinated presence and background
  selection as complete fold groups;

- `max_tss()` for finding the maximum true skill statistic;

- `tss_user_eval()` for use with ENMeval user-defined evaluation;

- `screen_egv_variance()` for use with SDMtune SWD to find variables
